# Axline Hooks Documentation

## Overview

Axline hooks allow you to execute custom scripts at specific points in the agentic workflow. Hooks can be placed in either:
- **Global hooks directory**: `~/Documents/Axline/Hooks/` (applies to all workspaces)
...
1. Open Axline settings in VSCode
...
- **Global Location**: `~/Documents/Axline/Hooks/TaskStart`
...
- **Global Location**: `~/Documents/Axline/Hooks/TaskResume`
...
- **Global Location**: `~/Documents/Axline/Hooks/TaskCancel`
...
- **Global Location**: `~/Documents/Axline/Hooks/TaskComplete`
...
- **Global Location**: `~/Documents/Axline/Hooks/PostToolUse`
...
- **Global Location**: `~/Documents/Axline/Hooks/PreCompact`
- **Workspace Location**: `.clinerules/hooks/PreCompact`

## Cross-Platform Hook Format

Axline uses a git-style approach for hooks that works consistently across all platforms:
...
Like git hooks, Axline executes hook files through a shell that interprets the shebang line:
- On Unix/Linux/macOS: Native shell execution with shebang support

This means:
- âœ?Same hook script works on all platforms
- âœ?Write once, run anywhere
- âœ?Use any scripting language (bash, node, python, etc.)
...
```bash
# Create hook file
nano ~/Documents/Axline/Hooks/PreToolUse

# Make executable
chmod +x ~/Documents/Axline/Hooks/PreToolUse
```
...
2. PreToolUse hook runs â†?can block or add context
...
2. PostToolUse hook runs â†?observes results
...
Axline supports two levels of hooks:

### Global Hooks
- **Location**: `~/Documents/Axline/Hooks/` (macOS/Linux)
...
1. The global hooks directory is automatically created at:
   - macOS/Linux: `~/Documents/Axline/Hooks/`
...
3. Enable hooks in Axline settings
...
```bash
#!/usr/bin/env bash
# ~/Documents/Axline/Hooks/PreToolUse
...
- Look for errors in VSCode's Output panel (Axline channel)
