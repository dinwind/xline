...
import { AxgateModeSelector } from "./AxgateModeSelector"
...
const ChatTextArea = forwardRef<HTMLTextAreaElement, ChatTextAreaProps>(
	(
		...
		const handleComposerConsumed = useCallback(
			(snapshot: { text: string; images: string[]; files: string[] }) => {
				setTimeout(() => {
					if ((textAreaRef.current?.value ?? "") === snapshot.text) {
						setInputValue("")
					}
					setSelectedImages((current) => (current === snapshot.images ? [] : current))
					setSelectedFiles((current) => (current === snapshot.files ? [] : current))
					textAreaRef.current?.focus()
				}, 100)
			},
			[setInputValue, setSelectedFiles, setSelectedImages],
		)

		useShortcut(usePlatform().togglePlanActKeys, axgateAuthEnabled ? () => {} : onModeToggle, {
				...
				<div
					...
					{!inputValue && selectedImages.length === 0 && selectedFiles.length === 0 && (
						<div className="text-xs absolute bottom-5 left-6.5 right-16 text-(--vscode-input-placeholderForeground)/50 whitespace-nowrap overflow-hidden text-ellipsis pointer-events-none z-1">
							{axgateAuthEnabled
								? "Add files, folders, docs… / for slash commands, hold shift to drag in files/images"
				...
				<div className="flex justify-between items-center -mt-[2px] px-3 pb-2">
					...
					<div className="relative flex-1 min-w-0 h-5">
						...
						<ButtonGroup className="absolute top-0 left-0 right-0 ease-in-out w-full h-5 z-10 flex items-center">
							{axgateAuthEnabled ? (
								<AxgateModeSelector
									composer={{ text: inputValue, images: selectedImages, files: selectedFiles }}
									onComposerConsumed={handleComposerConsumed}
								/>
							) : null}
							{axgateAuthEnabled ? (
								<ComposerIconButton
									...
									tooltip="Add files">
									<CursorIcon name="paperclip" size={14} />
								</ComposerIconButton>
							) : (
								<>
									<Tooltip>
							...
							<ServersToggleModal useComposerStyle={axgateAuthEnabled} />

							{!axgateAuthEnabled ? <ClineRulesToggleModal /> : null}
