...
const ClineRulesToggleModal: React.FC = () => {
	...
	const toggleRule = (isGlobal: boolean, rulePath: string, enabled: boolean) => {
		FileServiceClient.toggleClineRule(
			...
			.catch((error) => {
				console.error("Error toggling Axline rule:", error)
			})
	}
				...
				<PopupModalContainer $arrowPosition={arrowPosition} $menuPosition={menuPosition}>
					...
					<div className="flex-shrink-0 px-3 pt-2">
						...
						<div className="text-xs text-description mb-4">
							{currentView === "rules" ? (
								<p>
									Rules allow you to provide Axline with system-level guidance. Think of them as a persistent
								...
								<p>
									Skills are reusable instruction sets that Axline can activate on-demand. When a task matches a
									skill's description, Axline uses the <span className="font-bold">use_skill</span> tool to load
									the full instructions.
								</p>
							) : (
								<p>
									Hooks allow you to execute custom scripts at specific points in Axline's execution lifecycle,
