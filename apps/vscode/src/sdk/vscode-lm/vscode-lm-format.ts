...
export function asObjectSafe(value: unknown): object {
	...
	try {
		...
		Logger.warn("Axline <Language Model API>: Failed to parse object:", error)
		return {}
	}
}

function imagePlaceholder(mediaType?: string): vscode.LanguageModelTextPart {
	return new vscode.LanguageModelTextPart(`[Image (${mediaType || "unknown media-type"}) not supported by VSCode LM API]`)
}

/**
 * Render the content of a tool_result block into VS Code LM text parts.
 */
function toolResultTextParts(content: ContentBlock & { type: "tool_result" }): vscode.LanguageModelTextPart[] {
	...
	const isTypedBlock = (part: unknown): part is { type: "text" | "image" | "file" } =>
...
export function convertToVsCodeLmMessages(messages: Message[]): vscode.LanguageModelChatMessage[] {
	...
	for (const message of messages) {
		...
		if (message.role === "user") {
			const toolResults = message.content.filter(
				(part): part is ContentBlock & { type: "tool_result" } => part.type === "tool_result",
			)
			const nonTool = message.content.filter((part) => part.type === "text" || part.type === "image")

			const contentParts = [
				// Tool results first, as VS Code LM tool result parts.
				...toolResults.map(
					(toolResult) =>
						new vscode.LanguageModelToolResultPart(toolResult.tool_use_id, toolResultTextParts(toolResult)),
				),
		...
		if (message.role === "assistant") {
			const toolUses = message.content.filter(
				(part): part is ContentBlock & { type: "tool_use" } => part.type === "tool_use",
			)
			const nonTool = message.content.filter((part) => part.type === "text" || part.type === "image")
			...
			const contentParts = [
				// Tool calls first.
				...toolUses.map(
					(toolUse) => new vscode.LanguageModelToolCallPart(toolUse.id, toolUse.name, asObjectSafe(toolUse.input)),
				),
 ...
 * tool result parts.
 */
function endsWithToolResult(messages: vscode.LanguageModelChatMessage[]): boolean {
	const last = messages[messages.length - 1]
	if (!last || !Array.isArray(last.content) || last.content.length === 0) {
		return false
	}
	return last.content.some((part) => part instanceof vscode.LanguageModelToolResultPart)
}
